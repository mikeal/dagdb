...
import { fromIterable, read } from '@ipld/fbl/bare'
import { codec, hasher } from './block.js'
import { CID } from 'multiformats'

const isBlock = block => block.asBlock === block

const types = {}

const fblDecoder = (root, store) => {
  const get = store.get.bind(store)
  const iter = read(root, get)
  iter._dagdb = { v1: 'fbl', root }
  iter.encode = () => (async function * (r) { yield r })(root)
  iter.read = (...args) => read(root, get, ...args)
  return iter
}
...
const encode = async function * (value) {
  ...
  if (value[Symbol.asyncIterator] && !value._dagdb) {
    let last
    for await (const block of fromIterable(value, { codec, hasher })) {
      yield block
      last = block
    }
    yield { _dagdb: { v1: { fbl: last.cid } } }
    return
  }
